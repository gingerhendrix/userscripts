...
function getUserscriptUrl($file){
  global $useImportsService;
  global $importsServiceURL;
  global $baseUrl;
  global $localUrl;
  global $userscriptPreprocessor;
  global $useUserscriptPreprocessor;
  
  
  if($useUserscriptPreprocessor){
     $url =  $userscriptPreprocessor . $localUrl . $file ;
  }else{
    $url = $localUrl . $file . "?";
  }
  
  if($useImportsService){ 
     $url = $importsServiceURL . "?". $baseUrl . $url . "&" . time() . ".user.js";
  }else{
    $url =  $url . "&" . time() . ".user.js";
  }
...
<?php if($useUserscriptPreprocessor){ ?>
<span>Using preprocessor: <?php echo $userscriptPreprocessor ?></span>
<?php }?>
<div id="suites">
